...
extension ToolPermission {
    static let mockList: [ToolPermission] = [
        ...
        ToolPermission(id: "search_web", toolName: "search_web", displayName: "Поиск в интернете", toolDescription: "Ищет актуальную информацию в открытом интернете (через Wikipedia).", riskLevel: .low, state: .ask, requiresSystemPermission: nil),
        ToolPermission(id: "fetch_url", toolName: "fetch_url", displayName: "Открытие страниц из интернета", toolDescription: "Загружает текстовое содержимое конкретной веб-страницы по ссылке.", riskLevel: .medium, state: .ask, requiresSystemPermission: nil),

...
> **2026-07-15: Luma больше НЕ использует `.icon`-пакет.** Приложение
> перешло на классический `AppIcon.appiconset` с тремя universal-1024
> слотами (light/dark/tinted) в `Luma/Resources/AppIcon.xcassets`.
> Причина: формат `.icon` специализирует только default/dark/tinted, а
> режим Clear домашнего экрана для слоёных иконок система всегда рендерит
> стеклянным барельефом — сплошной белый глиф в Clear (как у классических
> иконок, например Grok) из `.icon` получить нельзя; из classic-иконки
> система строит Clear сама из tinted-слота. Рецепт ниже сохранён для
> будущих проектов, где нужен именно Liquid Glass-рендер.
